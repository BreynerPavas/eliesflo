...
function cUsuario(string $text, string $campo, array &$errores, int $max = 18, int $min = 1, bool $espacios = false, bool $case = TRUE) {
    $case = ($case === TRUE) ? "i" : "";
    $espacios = ($espacios === TRUE) ? " " : "";
    if ((preg_match("/^[a-z0-9ñ_-$espacios]{" . $min . "," . $max . "}$/u$case", sinTildes($text)))) {
        return true;
    }
    $errores[$campo] = "Error en el campo $campo";
    return false;
...
function cEmail(string $text, string $campo, array &$errores, bool $espacios = false, bool $case = TRUE) {
    $case = ($case === TRUE) ? "i" : "";
    $espacios = ($espacios === TRUE) ? " " : "";
    if ((preg_match("/^[a-zA-Z][a-zA-Z0-9-_]{1,29}@[a-zA-Z][a-zA-Z.]{1,15}.[a-zA-Z]{2,}$/", sinTildes($text)))) {
        return true;
    }
    $errores[$campo] = "Error en el campo $campo";
    return false;
...
function cPasword(string $text, string $campo, array &$errores) {
    if ((preg_match("/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,16}$/", sinTildes($text)))) {
        return true;
    }
    $errores[$campo] = "Error en el campo $campo";
    return false;
 ...
 */
 ...
 function cDescrUser(string $text, string $campo, array &$errores, int $max = 250, int $min = 1): bool
 {
     
     if ((preg_match("/^[a-zA-Z0-9 ]{" . $min . "," . $max . "}$/u", sinTildes($text)))) {
         return true;
     }
     //$errores[$campo] = "Error en el campo $campo";
     return false;
 ...
 * @return bool
 */
function cNum(string $num, string $campo, array &$errores, bool $requerido = TRUE, float $max = PHP_FLOAT_MAX): bool
{
    $cuantificador = ($requerido) ? "+" : "*";
    // Expresión regular para permitir números enteros y decimales con hasta dos decimales
    if (preg_match("/^[0-9]+(?:\.[0-9]{1,2})?" . $cuantificador . "$/", $num)) {
        // Convertir el número a float para asegurar la comparación con el máximo
        $numero = (float)$num;
        if ($numero <= $max) {
            return true;
        }
...
function pintaSelectTags(){
    $memeteca = new Memeteca(); // Debes crear una instancia de classMemeteca
    $tagsTotales = $memeteca->obtenerTagsJSON(); // Llamada correcta usando la instancia
    
    foreach ($tagsTotales as $id => $tag) {
        ...
        if (empty($errores)) {
             ...
             * */
                ...
                if (move_uploaded_file($directorioTemp, $nombreCompleto)) {
                    ...
                    $errores["$nombre"] = "Ha habido un error al subir el fichero1";
                    return false;
                }
            }else {
                $errores["$nombre"] = "Ha habido un error al subir el fichero2";
                return false;
            }
        }
    }
...
function cTags(string $texto, string $campo, array &$errores, bool $requerido = TRUE){

    $arrTags = explode("#",$texto);
    array_shift($arrTags);
    if(count($arrTags) >= 5){
        $errores[$campo] .= "Más de 5 tags";
        return false;
    }

    if($requerido && empty($arrTags)){
        $errores[$campo] = "No hay tags";
        return false;
    }

    foreach($arrTags as $tag){
        if(!preg_match("/[a-z0-9]{1,20}/",$tag)){
            $errores[$campo] = "Tag incorrecto: $tag";
            return false;
        }
    }
